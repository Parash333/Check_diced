...
<div class="container-fluid">
        ...
        $a = "SELECT * FROM TRADER WHERE TRADERID='$traderid'";
                    $b = oci_parse($connection,$a);
                    $c = oci_execute($b);
                    while($d = oci_fetch_assoc($b)){

                    echo'<h2>'.$d['TUSERNAME'].'</h2>';
...
<div class="col-12">
        <?php
            ...
            $a = "SELECT * FROM CUSORDER,CUSTOMER,PRODUCT,TRADER WHERE CUSORDER.PRODUCTID=PRODUCT.PRODUCTID  AND CUSORDER.CUSID = CUSTOMER.CUSID  AND PRODUCT.TRADERID=TRADER.TRADERID AND TRADER.TRADERID='$traderid'";
            $b = oci_parse($connection,$a);
            $c = oci_execute($b);
